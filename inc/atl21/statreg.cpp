...
static HKEY WINAPI HKeyFromString(__in_z LPTSTR szToken)
{
    struct keymap
    {
        LPCTSTR lpsz;
...
HRESULT CRegParser::NextToken(__out_ecount_z(MAX_VALUE_VALUE) LPTSTR szToken)
{
    USES_CONVERSION;
...
HRESULT CRegParser::AddValue(__in CRegKey& rkParent, __in_z_opt LPCTSTR szValueName, __out_ecount_z(MAX_VALUE_VALUE) LPTSTR szToken)
{
    USES_CONVERSION;
    HRESULT hr;
...
HRESULT CRegParser::SkipAssignment(__inout_ecount_z(MAX_VALUE_VALUE) LPTSTR szToken)
{
    HRESULT hr;
    TCHAR *szValue;
    szValue = (TCHAR *)malloc(sizeof(TCHAR) * MAX_VALUE);
...
HRESULT CRegParser::RegisterSubkeys(__in HKEY hkParent, __in BOOL bRegister, __in BOOL bRecover)
{
    CRegKey keyCur;
    TCHAR   *szToken;
    LONG    lRes;
...
HRESULT CRegParser::PreProcessBuffer(__in_z LPTSTR lpszReg, __deref_out_z LPTSTR* ppszReg)
{
    USES_CONVERSION;
...
HRESULT CRegParser::RegisterBuffer(__in_z LPTSTR szBuffer, __in BOOL bRegister)
{
...
LPCOLESTR CRegObject::StrFromMap(__in_z LPTSTR lpszKey)
{
    m_csMap.Lock();
