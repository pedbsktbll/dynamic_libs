    ...
    BOOL Add(T& t)
    {
        if(m_nSize == m_nAllocSize)
        {
            ...
            if (m_nAllocSize == 0) {
                ...
                SizeToAlloc = 2;
                if (SizeToAlloc > (SizeToAlloc * m_nSize)) {
                    return FALSE;
                }
                SizeToAlloc *= m_nSize;
            }

            if (SizeToAlloc > (SizeToAlloc * sizeof(T))) {
                return FALSE;
            }
            SizeToAlloc *= sizeof(T);

            if (SizeToAlloc > INT_MAX) {
                return FALSE;
            }

            aT = (T*)realloc(m_aT, SizeToAlloc);
            if(aT == NULL)
                return FALSE;

            m_nAllocSize = (int)SizeToAlloc;
