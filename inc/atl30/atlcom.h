                ...
                if (pMap[i].dwSizeData != 0)
                {
                        ...
                        if (SUCCEEDED(hr))
                        {
                                ...
                                case VT_UNKNOWN: {
                                    ...
                                    ATLASSERT(var.vt == VT_UNKNOWN || var.vt == VT_DISPATCH);  // var should always match
                                    if ((var.punkVal == NULL) || 
                                        (pMap[i].piidDispatch == NULL) ||
                                         FAILED(var.punkVal->QueryInterface(*(pMap[i].piidDispatch), (void **) pData))) 
                                    {
                                        *reinterpret_cast<LPUNKNOWN *>(pData) = NULL;
                                    }
                                } break;
#endif
                                case VT_DISPATCH: {
                                    ...
                                    if ((var.pdispVal == NULL) ||
                                        FAILED(var.pdispVal->QueryInterface(__uuidof(IDispatch), (void **) pData))) 
                                    {
                                        *reinterpret_cast<LPUNKNOWN *>(pData) = NULL;
                                    }
                                } break;
...
public:
        STDMETHOD(EnumConnectionPoints)(IEnumConnectionPoints** ppEnum)
        {
                ...
                *ppEnum = NULL;
                ...
                UINT SizeToAlloc = nCPCount;

                if (SizeToAlloc > (SizeToAlloc * sizeof(IConnectionPoint*)))
                {
                    delete pEnum;
                    return HRESULT_FROM_WIN32(ERROR_ARITHMETIC_OVERFLOW);
                }

                SizeToAlloc *= sizeof(IConnectionPoint*);

                IConnectionPoint** ppCP = (IConnectionPoint**)_ATL_SAFE_ALLOCA(SizeToAlloc, _ATL_SAFE_ALLOCA_DEF_THRESHOLD);
                if (ppCP == NULL)
                {
                        delete pEnum;
                        return E_OUTOFMEMORY;
