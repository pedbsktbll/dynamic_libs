...
public:
        ...
        HRESULT  PreProcessBuffer(__in_z LPCTSTR lpszReg, __deref_out_z LPTSTR* ppszReg);
        HRESULT  RegisterBuffer(__in_z LPCTSTR szReg, BOOL bRegister);

protected:

        void    SkipWhiteSpace();
        HRESULT NextToken(__out_ecount_z(MAX_VALUE_VALUE) LPTSTR szToken);
        HRESULT AddValue(__in CRegKey& rkParent, __in_z_opt LPCTSTR szValueName, __out_ecount_z(MAX_VALIE_VALUE) LPTSTR szToken, bool bQuoteModule = FALSE);
        ...
        static LPCTSTR StrChr(__in_z LPCTSTR lpsz, __in TCHAR ch);
...
inline LPCTSTR CRegParser::StrChr(__in_z LPCTSTR lpsz, __in TCHAR ch)
{
        LPCTSTR p = NULL;
        if (lpsz == NULL)
                return NULL;
...
inline HRESULT CRegParser::AddValue(__in CRegKey& rkParent, __in_z_opt LPCTSTR szValueName, __out_ecount_z(MAX_VALIE_VALUE) LPTSTR szToken, bool bQuoteModule)
{
        USES_CONVERSION_EX;
        HRESULT hr;
...
inline HRESULT CRegParser::PreProcessBuffer(__in_z LPCTSTR lpszReg, __deref_out_z LPTSTR* ppszReg)
{
    USES_CONVERSION_EX;
...
inline HRESULT CRegParser::RegisterBuffer(__in_z LPCTSTR szBuffer, __in BOOL bRegister)
{
        TCHAR*  szToken;
        HRESULT hr = S_OK;
